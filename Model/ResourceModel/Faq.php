...
namespace Alsurmedia\Faq\Model\ResourceModel;
...
use Alsurmedia\Faq\Model\Config\Source\Urlkey;
...
class Faq extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    const FAQ_REQUEST_PATH = 'faq'.Faq::FAQ_DOT_HTML;
    ...
    {
        $this->_init('alsurmedia_faq', 'faq_id');
    }
    ...
    protected function _afterLoad(AbstractModel $object)
    {
        $faq_store = $this->getConnection()
            ->select()
            ->from($this->getTable('alsurmedia_faq_store'), ['store_id'])
            ->where('faq_id = :faq_id');
        ...
        $faq_category = $this->getConnection()
            ->select()
            ->from($this->getTable('alsurmedia_faq_category_id'), ['category_id'])
            ->where('faq_id = :faq_id');
    ...
    protected function duplicateFaqIdentifier(AbstractModel $object)
    {
        ...
        $select = $this->getConnection()->select()
            ...
            ->join(
                ['faq_store' => $this->getTable('alsurmedia_faq_store')],
    ...
    public function getFaqcategory($faq_id = null)
    {
        $select = $this->getConnection()->select()
            ...
            ->joinLeft(
                ['faqcategory' => $this->getTable('alsurmedia_faq_category_id')],
                'faq.faq_id = faqcategory.faq_id',
                ['category_id']
            )
            ->joinLeft(
                ['cat' => $this->getTable('alsurmedia_faq_category')],
                'faqcategory.category_id = cat.category_id',
                ['title', 'identifier']
            )
            ->joinLeft(
                ['cat_store' => $this->getTable('alsurmedia_faq_category_store')],
                'faqcategory.category_id = cat_store.category_id',
                ['store_id']
            )
    ...
    public function getRelatedQuestion($faq_id = null, $category_id = null)
    {
        $select = $this->getConnection()->select()
            ...
            ->where('faq.faq_id <> ?', $faq_id)
            ->where('faq.is_active = ?', '1')
            ->where('faqcategory.category_id = ?', $category_id)
    ...
    public function updateNumberOfFaqsInCategory()
    {
        ...
        $select = $adapter->select()->from($this->getTable('alsurmedia_faq_category_id'), ['count' => 'COUNT(category_id)', 'category_id'])->group('category_id');

        $faq_category_results = $this->getConnection()->fetchAll($select);

        $adapter->update($this->getTable('alsurmedia_faq_category'), ['count' => '0']);

        foreach ($faq_category_results as $value) {
            $adapter->update($this->getTable('alsurmedia_faq_category'), ['count' => $value['count']], ['category_id = ?' => (int) $value['category_id']]);
        };
    ...
    protected function saveFaqRelation(AbstractModel $object)
    {
        ...
        if ($faq_id && (int) $faq_id > 0) {
            ...
            if ($category_id) {
                ...
                $adapter->delete($this->getTable('alsurmedia_faq_category_id'), $condition);

                $faq_category = [
                    'faq_id' => (int) $faq_id,
                    'category_id' => (int) $category_id
                ];
                $adapter->insertMultiple($this->getTable('alsurmedia_faq_category_id'), $faq_category);
            ...
            if ($stores) {
                $condition = ['faq_id = ?' => (int) $faq_id];
                $adapter->delete($this->getTable('alsurmedia_faq_store'), $condition);
                ...
                $adapter->insertMultiple($this->getTable('alsurmedia_faq_store'), $data);
