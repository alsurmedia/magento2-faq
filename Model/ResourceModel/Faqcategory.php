...
class Faqcategory extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    const FAQ_CATEGORY_FILE_PATH_ACCESS = 'alsurmedia/faq/category/';
    ...
    {
        $this->_init('alsurmedia_faq_category', 'category_id');
    }
    ...
    protected function _afterLoad(AbstractModel $object)
    {
        $category_store = $this->getConnection()
            ->select()
            ->from($this->getTable('alsurmedia_faq_category_store'), ['store_id'])
    ...
    protected function saveCategoryRelation(AbstractModel $object)
    {
        ...
        if ($category_id && (int) $category_id > 0) {
            ...
            if ($stores) {
                ...
                $adapter->delete($this->getTable('alsurmedia_faq_category_store'), $condition);

                $entity_type  = Faqcategory::FAQ_CATEGORY_ENTITY_TYPE;
                ...
                $adapter->insertMultiple($this->getTable('alsurmedia_faq_category_store'), $data);
    ...
    public function getFaqcategoryStore($category_id = null)
    {
        ...
        $select = $this->getConnection()->select()
            ->from(['faqcategory' => $this->getMainTable()])
            ->joinLeft(
                ['faqcat_store' => $this->getTable('alsurmedia_faq_category_store')],
                'faqcategory.category_id = faqcat_store.category_id',
                ['store_id']
            )
            ->where('faqcategory.category_id = ?', $category_id)
            ->where('faqcategory.is_active = ?', '1')
            ->where('faqcat_store.store_id IN (?)', $storeIds)
            ->group('faqcategory.category_id')
            ->limit(1);
    ...
    public function getCategoriesList()
    {
        ...
        $select = $adapter->select()
            ->from(['cat' => $this->getTable('alsurmedia_faq_category')])
            ->join(['cat_store' => $this->getTable('alsurmedia_faq_category_store')], 'cat.category_id = cat_store.category_id', ['store_id'])
